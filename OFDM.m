%Funcion OFDM: modula un numero de bits nsize, y los pasa por un canal 
%El ancho de banda BW_ofdm puede tener un valor de 3000 o 6000
%bool indica con 1 adicion de AWGN al canal, 0 para un canal sin ruido AWGN
%bool2 indica con 1 multicamino en e lcanal, 0 para un canal sin
%multicamino
...
% data_zero = zeros(1,length(data_bi)+constlen);  %ANADIMOS ZEROS PARA INICIALIZAR LOS REGISTROS
% data_zero(constlen+1:end) = data_bi(1:end);
% codedata = convenc(data_zero, trellis);  %convolucion 
% %anadir X ceros al principio
...
% data_zero = zeros(1,length(codedata)+nzeros);   %Anadimos tantos ceros como delay tiene el intrl
...
zero_padding = 0;
while rem(length(intrlv_data),(n_sc*4)) ~= 0 %Creamos un tamano de vector factor del tamano de bits de simbolo
...
%PILOTS
Nt = (floor(1/(Tsofdm*T_dopp))-1); %Separacion de pilotos maxima
Npilots = ceil(n_colum/(Nt-1)); % Numero de pilotos total
n_colum_pilots = n_colum + Npilots; % Numero de columnas totales
Pilot_value = 3+3j; %VALOR FIJADO POR NOSOTROS
column_padding =0;

%Fijamos un pilot al final de la matriz para mejorar la interpolacion posterior
%Guardamos en una variable el numero de columnos de zeros añadidas para
%despues quitarlas
...
% El ultimo de todos es el que termina
...
n_colum_pilots = n_colum_pilots +1;  
matrix_CN = zeros((n_sc+1),n_colum_pilots); %Anadimos Carrier NULL 
matrix_CN(2:end,1:end)= matrix_final(1:end,1:end);

matrix_ifft = ifft(matrix_CN,[],1);  %IFFT

%figure(2)      %%para comprobar que la matrix efectivamente es ortogonal
...
%INTERP USAR PARA SUBIR EL BW DE LA SENAL 
...
for n = 1:length(SNR)
  ...
  %% RECEIVER  
    ...
    signal_decimate = downsample(signal_noisy,ceil(f_sample/BW_ofdm));  %Diezmamos la senal
    ...
    %Equalizacion del canal 
    H = Pilot_final./Pilot_orig;
    
    for i = (1:n_sc)
        H_freq = H(i,1:end); %obtenemos los pilots
        %Aqui Interpolamos
        H_interp(i,1:(n_colum_pilots))= interp1((1:Npilots+1),H_freq,(1:(1/Nt):Npilots+1));       
    end
    
    TX = signal_demod_CN./H_interp;
    signal_final= zeros(n_sc,n_colum);
    m = 1;
    for i=(1:Nt:n_colum_pilots - 1)
        for j=(i+1:(i+Nt -1))
            signal_final(1:end,m)=TX(1:end,j);
            m = m+1;
        end
    end
    signal_final= signal_final(1:end,1:n_colum); % quitamos los 0
    %DEMOD
